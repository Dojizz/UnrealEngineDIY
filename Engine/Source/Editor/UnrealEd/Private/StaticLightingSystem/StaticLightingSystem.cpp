...
// 构建一个新的StaticLightingSystem，利用当前的world+level数据和option，这个system会开始lightmass process
...
// 构建lightmass processor，整理数据，大部分的流程都在这里，最后的结果是进入AmortizedExport stage，可见UpdateLightingBuild
...
// 创建一个lightmass processor，被定义在lightmass.cpp中，这个构造函数会与swarm建立连接，并构建其自身拥有的exporter用于导出数据
...
// 从之前create的LightmassProcessor处取得其exporter并向其中写入数据
...
// lightmass processor先create -> gather scene -> initiate
bool FStaticLightingSystem::InitiateLightmassProcessor()
{
	...
	if ( !GEditor->GetMapBuildCancelled() )
	{
...

		{
			...
			bOpenJobSuccessful = LightmassProcessor->OpenJob(); // 调用swarm接口，输入scene guid开始job
		}

		if (bOpenJobSuccessful)
		{
			LightmassProcessor->InitiateExport(); // 初始化export，之后editor可以回归主循环，并根据CurrentBuildStage执行不同的操作
			bSuccessful = true;
...
// 该函数会在主循环中被调用，从进行amortized export开始
