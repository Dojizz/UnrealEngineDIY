...
void FStaticLightingSystem::InitializePhotonSettings()
{
	...
	// direct中有多少photon用作irradiance photon
...
#endif
	// 通过density+前面计算的surface area计算indirect photon path数
	...
	// 同样的原理，计算second pass photon数(虽然我还不理解这是什么
...
void FStaticLightingSystem::EmitPhotons()
{
	...
	// 处理直接光子，产生direct photon map + indirect path ray + 一部分irradiance photon(由direct photon产生的)
	...
	// 但事实上似乎只有directional light使用了cathe path ray作为indirect photon的方向？
	for (int32 LightIndex = 0; LightIndex < Lights.Num(); LightIndex++)
	{
	...
	// 处理间接光子，使用直接光子所生成的indirect path rays(但似乎只有方向光会利用这个信息，其他光源还是从自身处重新采样光线)
	...
	// 处理emit photon生成的irradiance photons
	if (PhotonMappingSettings.bUseIrradiancePhotons)
	{
		...
		// 对irradiance photons做预处理的标记，有助于提高照度估计的准确度
		...
		{ // 对每个纹素都预先找到离其最近的那个irradiance photon
	...
	TArray<TArray<FIrradiancePhoton>>& IrradiancePhotons)
{
	...
	const FDirectPhotonEmittingInput Input(ImportanceBounds, LightDistribution); // bounds是一片限定区域(应该是球+box)，distri存储每个当前的cdf，用这两个数据构建input

	int32 NumIndirectPhotonPathsRemaining = NumIndirectPhotonPaths; // lighting system中预先设置好的indirect光子路径数，263，应当是总的path数
	// Setup work ranges, which are sections of work that can be done in parallel.
	DirectPhotonEmittingWorkRanges.Empty(NumPhotonWorkRanges); // workrange=256
	for (int32 RangeIndex = 0; RangeIndex < NumPhotonWorkRanges - 1; RangeIndex++) // 处理前面255个workrange
	{
		int32 NumIndirectPhotonPathsRange = FMath::Max(NumIndirectPhotonPaths / NumPhotonWorkRanges, 1); //计算每个workrange负责几个path

		if (NumIndirectPhotonPathsRemaining == 0)
		{
			NumIndirectPhotonPathsRange = 0; // 若remain的path数为0，则该workrange的path num=0
		}

		NumIndirectPhotonPathsRemaining = FMath::Max(NumIndirectPhotonPathsRemaining - NumIndirectPhotonPathsRange, 0); //更新remain的path数
		// 这个direct photon workrange包含两部分，一部分是划分成256份的direct photon，一部分是划分成256份的indirect path，都存在一起
	...
	// The last work range contains the remainders，最后一个workrange要处理剩下的所有photons+path
	...
	{
		// Initialize outputs with the appropriate irradiance photon array，通过array的index初始化output
		DirectPhotonEmittingOutputs.Add(FDirectPhotonEmittingOutput(&IrradiancePhotons[IrradianceArrayStart + RangeIndex]));
	}

	// Spawn threads to emit direct photons，利用thread index+之前创建的input信息(bounds+cdf)创建线程
	...
	// 使用octree，以bounds的中心建树，暂不清楚direct和irradiance的区别
	...
	while (DirectPhotonEmittingWorkRangeIndex.GetValue() < DirectPhotonEmittingWorkRanges.Num()
		|| NextOutputToProcess < DirectPhotonEmittingOutputs.Num()) // 在达到需要的光子数前，不断地处理output，并把photon加入octree中
	{
		// Process one work range on the main thread，这个函数会在整个workrange执行完后，去处理output
...
void FStaticLightingSystem::EmitDirectPhotonsThreadLoop(const FDirectPhotonEmittingInput& Input, int32 ThreadIndex)
{
	while (true)
	{
		...
		if (RangeIndex < DirectPhotonEmittingWorkRanges.Num()) // 开256个子任务，num=256
		{
	...
	FDirectPhotonEmittingOutput& Output)
{
	...
	// Emit photons until we reach the limit for this work range, 每次循环发射一个photon
		...
		|| NumIndirectPathRaysGathered < WorkRange.TargetNumIndirectPhotonPaths)
	{
		...
		// Register this photon path as long as it hit a frontface of something in the scene，击中+击中的是正面
			...
			&& Dot3(WorldPathDirection, PathIntersection.IntersectionVertex.WorldTangentZ) < 0.0f)
		{
				...
				&& Input.ImportanceBounds.GetBox().IsInside(PathIntersection.IntersectionVertex.WorldPosition))
			{
				...
				if (PhotonMappingSettings.bUseIrradiancePhotons 
					// Create an irradiance photon for a fraction of the direct photons，抽样一部分photon加入irradiance photons
			...
			// Continue tracing this path if we don't have enough indirect photon paths yet，是否需要发射间接光子路径
			if (NumIndirectPathRaysGathered < WorkRange.TargetNumIndirectPhotonPaths)
			{
				...
				// Generate a new path direction from the BRDF 求解交点处的BRDF颜色+采样出射方向
	...
	FIndirectPhotonEmittingOutput& Output)
{
	...
	// Emit photons until we reach the limit for this work range, 开始一次间接光子发射
	while (bIndirectPhotonsNeeded && Output.NumPhotonsEmitted < WorkRange.NumIndirectPhotonsToEmit)
	{
		...
		// Find the first vertex of the photon path，与场景求交
		FLightRayIntersection PathIntersection;
		...
		// Continue tracing this photon path as long as it hit a frontface of something in the scene，判断是否物体正面，针对当前光子，进入处理其对应的indirect photon的循环
		while (PathIntersection.bIntersects && Dot3(WorldPathDirection, PathIntersection.IntersectionVertex.WorldTangentZ) < 0.0f)
		{
			...
			// Only deposit photons inside the importance bounds，判断交点是否在bound内
			if (Input.ImportanceBounds.GetBox().IsInside(PathIntersection.IntersectionVertex.WorldPosition))
			{
				// Only deposit a photon if it is not a direct lighting path, and we still need to gather more indirect photons，只能加入间接光子，number记录bounce次数
					...
					&& Output.NumPhotonsEmitted < WorkRange.NumIndirectPhotonsToEmit)
				{
					...
					{ 
						// This is a first bounce photon，相对于间接光子的"first bounce"
					...
					{
						Output.SecondBouncePhotons.Add(NewPhoton); // 否则加入后续bounce的光子
...
#endif
					// Create an irradiance photon for a fraction of the deposited photons，抽样是否加入irradiance光子，ir光子本身此时只记录了位置信息
					if (PhotonMappingSettings.bUseIrradiancePhotons 
						&& bShouldCreateIrradiancePhoton
			...
			// But with final gathering, a first bounce photon is second bounce lighting，这个pathvertices实际上就可以理解为完整的bounce次数
			...
			// 如果没到bounce次数限制，那就根据交点的brdf继续向下trace
			FStaticLightingVertex IntersectionVertexWithTangents(PathIntersection.IntersectionVertex);
			...
			if (NumberOfPathVertices == 1)
			{
				...
				// Re-weight the throughput based on the probability of surviving. 第一次bounce不使用RR，保证产生indirect photon
			...
			// Trace a ray to determine the next vertex of the photon's path. trace这条下一个方向的ray
			SampleRay.TraceFlags |= LIGHTRAY_FLIP_SIDEDNESS;
		...
		// First bounce escaped photon 特别处理一次bounce直接出去的结果
		if (!PathIntersection.bIntersects 
